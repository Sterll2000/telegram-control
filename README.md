...
## Автоматизация списка админов Iris

Источник автоматических ролей — ответы Iris в двух чатах:

- ⚫ `IRIS_CHAT_BLACK` — Чёрный чат
- 🔵 `IRIS_CHAT_BLUE` — Синий чат

`scripts/iris-monitor.mjs` использует MTProto user session и polling Telegram history. Он не зависит от `NewMessage`/`Raw`, которые на некоторых аккаунтах не доставляли updates. Worker: 

1. находит ответы Iris по реальному Telegram ID `5443619563`;
2. извлекает Telegram ID администраторов из mention entities, `@username` или точного имени в чате;
3. отправляет snapshot + Telegram IDs на `/api/admins/internal/sync`;
4. сайт связывает запись Iris с пользователем по Telegram ID;
5. при следующем входе через Telegram `initData` сервер автоматически выдаёт соответствующие звёзды/роль;
6. если Iris убрал пользователя, автоматическая Iris-роль отзывается, а ручная роль (`MANUAL`) сохраняется.

Для полного автоматического цикла `iris:monitor` может сам отправлять `кто админ` через Bot API. Это включается настройкой `IRIS_AUTO_REQUEST_ENABLED=true` или переключателем автоматизации в админ-панели. Отдельный `iris:bot` оставлен как requester-only совместимый worker.

Подробная production-настройка находится в `DEPLOYMENT.md`.
